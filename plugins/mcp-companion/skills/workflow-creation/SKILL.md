...
description: >
   ...
   If the user wants to model from an existing or legacy codebase, recommend
   using `/mcp-companion:extract-aggregate` first (one aggregate at a time,
   typically one workflow per aggregate), then continue with this skill.
...
First recommend `/mcp-companion:extract-aggregate` as a planning step:
