...
For each domain event, the read model defines a query that displays the current system state relevant to the actor BEFORE invoking the command that triggers the event. It provides the background information the actor needs to make decisions and take action (trigger the command with required input data).

## Field Design

### Simple Fields
Primitive values returned by the query. Tag with `isFilter: true` if the field is a search/filter parameter:
```json
[
...
When a query attribute references an entity in another bounded context, always use `Id` as suffix and never include nested fields (avoid single queries that cross multiple bounded contexts):
```json
[
...
When the response includes nested levels of data from related entities or value objects, set `relatedEntity` and include nested sub-fields. Name the field as the entity / VO in camelCase (must NOT have an "Id" suffix):
